...
public sealed partial class MainWindow : Window
{
    ...
    public MainWindow(ConnectionsViewModel viewModel)
    {
        ...
        catch (Exception exception) { AppLog.Error("Mica backdrop initialization failed; continuing without Mica", exception); }
        try { AppWindow.Resize(new SizeInt32(1200, 760)); }
        catch (Exception exception) { AppLog.Error("Initial window resize failed; continuing with system default size", exception); }
    ...
    {
        if (LogsPage.Visibility == Visibility.Visible && ViewModel.LogAutoRefresh && !ViewModel.IsBusy)
    ...
    private async void Navigation_SelectionChanged(NavigationView sender, NavigationViewSelectionChangedEventArgs args)
    {
        ...
        if (tag == "logs") await ViewModel.RefreshLogAsync();
    ...
    private async void ConnectionsList_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        if (LogsPage.Visibility == Visibility.Visible) await ViewModel.RefreshLogAsync();
    }
    ...
    private async void EditProfile_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            await ViewModel.SaveSelectedProfileAsync(
                ...
                tunnelId.Text,
                target.Text,
                new ProfilePreference { Enabled = enabled.IsChecked == true, AutoConnect = autoConnect.IsChecked == true, AutoReconnect = autoReconnect.IsChecked == true },
                secret.Password,
    ...
    private async void BrowseTunnelClient_Click(object sender, RoutedEventArgs e)
    {
        ...
        catch (Exception exception) { await ShowErrorAsync($"选择 tunnel-client 失败：{exception.Message}"); }
    }
    ...
    private void SetupTrayIcon()
    {
        ...
        try
        {
            ...
            menu.Items.Add("打开", null, (_, _) => DispatcherQueue.TryEnqueue(RestoreFromTray));
            menu.Items.Add(new System.Windows.Forms.ToolStripSeparator());
            menu.Items.Add("退出", null, (_, _) => DispatcherQueue.TryEnqueue(() => _ = ExitApplicationAsync()));
            _trayIcon = new System.Windows.Forms.NotifyIcon
            {
                Text = "OpenAI MCP Tunnel Manager",
