...
适用于 OpenAI MCP Tunnel Manager 1.2.0 及后续 WinUI 3 版本。
...
- [x] tunnel-client 常态只占紧凑设置项：状态摘要 + 来源下拉 + 单一主按钮 + “更多”菜单。
- [x] 托管模式未安装显示“下载”，已安装显示“更新”；不显示“已是最新”按钮。
- [x] 点击“更新”才检查远端版本，应用启动不自动联网检查。
- [x] 下载/安装期间才显示进度条与阶段文本，下载阶段可取消，结束后自动恢复紧凑高度。
- [x] 来源下拉只展示当前模式对应操作，不同时堆叠托管与自定义控件。
- [x] 自定义 EXE 选择后自动验证；无单独“验证”按钮，失败时不覆盖当前配置并可重新选择。
- [x] “更多”菜单包含打开文件位置和托管旧版本清理；自定义模式隐藏托管清理项。
...
- [ ] Narrow `<650` 下连接详情、日志过滤、tunnel-client 紧凑设置项和高级目录控件可完整操作。
